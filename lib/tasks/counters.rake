namespace :ktv do
  task :counters => :environment do 
    User.all.each{|x| print x.coursewares_uploaded_count.to_s;x.ua(:coursewares_count,0);x.ua(:coursewares_uploaded_count,Courseware.non_redirect.nondeleted.normal.is_father.where(:uploader_id=>x.id).count);print "--#{x.coursewares_uploaded_count}\n"}
