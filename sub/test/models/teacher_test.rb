...
describe Teacher do
  ...
  it "老师的一阶索引" do
    pl = Teacher.new
    title = "pppppssssssvvvvvvrrrrrtchtch#{Teacher.count+1}"
    pl.name = title
    pl.save(:validate=>false)
    refute Redis::Search.query("Teacher", title).try(:[],0).try(:[],'id') == pl.id.to_s, '信息不全，不建立一阶索引'
    pl.update_attribute(:department_fid,Department.nondeleted.gotfid.first.fid)
    assert Redis::Search.query("Teacher", title).try(:[],0).try(:[],'id') == pl.id.to_s, '信息齐全后，保存即建立这个老师的一阶索引'
    
    title2 = title.reverse
    assert title2!=title
    pl.update_attribute(:name, title2)
    refute Redis::Search.query("Teacher", title).try(:[],0).try(:[],'id') == pl.id.to_s, '标题改了，老标题索引不再存在'    
