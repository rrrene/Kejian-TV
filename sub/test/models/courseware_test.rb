...
describe Courseware do
  ...
  def test_normal_after_save_coursewares_uploaded_count
    ...
    @user1.reload
    @user2.reload
    assert user1_coursewares_uploaded_count_before + 1 == @user1.coursewares_uploaded_count
    assert user2_coursewares_uploaded_count_before + 1 == @user2.coursewares_uploaded_count
    @courseware.uploader_id = @user2.id
    @courseware.save(:validate=>false)
    @user1.reload
    @user2.reload
    assert user1_coursewares_uploaded_count_before == @user1.coursewares_uploaded_count
    assert user2_coursewares_uploaded_count_before == @user2.coursewares_uploaded_count
  end
  ...
  def test_disliked_then_thanked_by_user
    ...
    @user2.reload
    @courseware.reload
    assert user1_dislike_coursewares_count + 1 == @user1.dislike_coursewares_count
    assert user2_disliked_coursewares_count + 1 == @user2.disliked_coursewares_count
