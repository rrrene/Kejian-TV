            ...
            while !done
              begin
                ...
                if 0==i
                  ...
                  puts `convert "#{pic}"  -bordercolor white -border 0 -background white -alpha remove  -alpha off -resize 222x +repage -gravity North "#{pinpic}"`
                  inf = `identify "#{pinpic}"`
                  if inf=~/JPEG (\d+)x(\d+)/
                ...
                puts `convert "#{pic}" -bordercolor white -border 0 -background white -alpha remove -alpha off -thumbnail '210x>' -crop 210x158+0+0 +repage -gravity North "#{pic2}"`
                done = true
              rescue => e
                puts e
              end
