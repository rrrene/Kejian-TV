...
//      return new Element('img', { src: img.src, 'class': 'ajax-loader' })
//    }
//
...
function check_password(){
	var len=$('newuser_password').value.length;
	if(len<6)
	{
		$('check_password_output').innerHTML = '<img src="/images/wrong.gif" />密码太短'
	}else if(len >20){
		$('check_password_output').innerHTML = '<img src="/images/wrong.gif" />密码太长'
	}else{
		$('check_password_output').innerHTML = '<img src="/images/ok.gif" />'
	}
}

function check_password_conf()
{
	if($('newuser_password_conf').value!=$('newuser_password').value){
		$('check_password_conf_output').innerHTML = '<img src="/images/wrong.gif" />密码不相符'
	}else{
		$('check_password_conf_output').innerHTML = '<img src="/images/ok.gif" />'
	}
}
...
function check_email()
{
                var at="@"
		var dot="."
		var lat=$('newuser_email').value.indexOf(at)
		var lstr=$('newuser_email').value.length
		var ldot=$('newuser_email').value.indexOf(dot)
		if ($('newuser_email').value.indexOf(at)==-1){
		   $('check_email_output').innerHTML = '<img src="/images/wrong.gif" />非法Email'
		   
		}else if ($('newuser_email').value.indexOf(at)==-1 || $('newuser_email').value.indexOf(at)==0 || $('newuser_email').value.indexOf(at)==lstr){
		   $('check_email_output').innerHTML = '<img src="/images/wrong.gif" />非法Email'
		   
		}else if ($('newuser_email').value.indexOf(dot)==-1 || $('newuser_email').value.indexOf(dot)==0 || $('newuser_email').value.indexOf(dot)==lstr){
		    $('check_email_output').innerHTML = '<img src="/images/wrong.gif" />非法Email'
		    
		}else if ($('newuser_email').value.indexOf(at,(lat+1))!=-1){
		    $('check_email_output').innerHTML = '<img src="/images/wrong.gif" />非法Email'
		    
		 }else if ($('newuser_email').value.substring(lat-1,lat)==dot || $('newuser_email').value.substring(lat+1,lat+2)==dot){
		    $('check_email_output').innerHTML = '<img src="/images/wrong.gif" />非法Email'
		    
		 }else if ($('newuser_email').value.indexOf(dot,(lat+2))==-1){
		    $('check_email_output').innerHTML = '<img src="/images/wrong.gif" />非法Email'
		 }else if ($('newuser_email').value.indexOf(" ")!=-1){
		    $('check_email_output').innerHTML = '<img src="/images/wrong.gif" />非法Email'
		 }else{
                    $('check_email_output').innerHTML = '<img src="/images/ok.gif" />'
                 }
}

var course = {
  validate: function() {
    var Submit = $('course_submit');
    var errors = "";
    if (errors == "") {
  ...
  */
 	...
 	// Default Options
		var defaultOptions = {
			...
			boxImage	: '/progress_bar/percentImage.png',			// boxImage : image around the progress bar
			barImage	: '/progress_bar/percentImage_back.png',	// Image to use in the progressbar. Can be an array of images too.
